...
func (s *ServerSession) IsSingleConnect() bool {
	// 检查客户端是否在第一个数据包中设置了标志位
	if s.p == nil || len(s.p) < hdrLen {
		return false
	}
	return s.p[hdrFlags]&hdrFlagSingleConnect != 0
}
...
func (h *ServerConnHandler) handleAuthenStart(ctx context.Context, s *ServerSession) ([]byte, error) {
	...
	if err != nil {
		err = fmt.Errorf("Bad ServerAddr AuthenReply: %s", err)
	}
	return s.p, err
}
...
func (h *ServerConnHandler) handleAuthorRequest(ctx context.Context, s *ServerSession) ([]byte, error) {
	...
	s.p, err = reply.marshal(s.p[:hdrLen])
	if err != nil {
		err = fmt.Errorf("Bad ServerAddr AuthorResponse: %s", err)
	}
	return s.p, err
}
...
func (h *ServerConnHandler) handleAcctRequest(ctx context.Context, s *ServerSession) ([]byte, error) {
	...
	s.p, err = reply.marshal(s.p[:hdrLen])
	if err != nil {
		err = fmt.Errorf("Bad ServerAddr AcctReply: %s", err)
	}
	return s.p, err
}
	...
	defer func() {
		if r := recover(); r != nil {
			remoteAddr := func() string {
				defer func() { recover() }()
				if sess.c.nc != nil {
					if addr := sess.c.nc.RemoteAddr(); addr != nil {
						return addr.String()
					}
				}
				return "unknown"
			...
			localAddr := func() string {
				defer func() { recover() }()
				if sess.c.nc != nil {
					if addr := sess.c.nc.LocalAddr(); addr != nil {
						return addr.String()
					}
				}
				return "unknown"
			}()
			sessErr := fmt.Errorf("TACACS+ server running has panic:%v, remoteAddr:%s, localAddr:%s", r, remoteAddr, localAddr)
			fmt.Println(sessErr)
		}
	}()
	...
	for {
		...
		go func(conn net.Conn) {
			...
			if tcpConn, ok := conn.(*net.TCPConn); ok {
				_ = tcpConn.SetNoDelay(true)   // 禁用Nagle算法，减少延迟
				_ = tcpConn.SetKeepAlive(true) // 启用Keep-Alive
